      ...
      describe Clickhouse::Connection::Client do
        ...
        describe "#get" do
          it "sends a GET request the server" do
            ...
            client.expects(:get).with("/?query=foo&output_format_write_statistics=1&qparam=bar", nil).returns(stub(:status => 200, :body => ""))
            @connection.stubs(:log)
            @connection.get("foo", qparam: "bar")
          end
        end
        ...
        describe "#post" do
          it "sends a POST request the server" do
            @connection.instance_variable_set :@client, (client = mock)
            client.expects(:post).with("/?query=foo&output_format_write_statistics=1&qparam=bar", "body").returns(stub(:status => 200, :body => ""))
            @connection.stubs(:log)
            @connection.post("foo", { qparam: "bar" }, "body")
          end
        end
